...
var win = floaty.rawWindow(
    <frame id="parent" >
        <img id="img" gravity="center" bg="#00000000"/>
    </frame>
);
...
function settouch(value) {
    ui.post(()=>{
    value ? win.setPosition(0,0) : win.setPosition(-9999,-9999)
   // value ? win.setSize(-1, -1) : win.setSize(0, 0);
    })
}
...
var mWindow,
    params;
mWindow = getClassField(win, "mWindow");
log(mWindow); // com.stardust.autojs.core.floaty.RawWindow@26b1509
params = mWindow.getWindowLayoutParams();
//设置全屏悬浮窗
/*
params.flags |= 
...
var win_ = floaty.rawWindow(
    <vertical marginTop="20">
        ...
        <horizontal w="200" margin="-5">
            <button id="but_y" layout_weight="2" text="移动"/>
...
function reDraw(canvas) {
    ...
    var AX = w / 2;
    var AY = h / 2;
    if(win.getHeight()==device.width&&w == device.width){
    af = new 绘布(win.img)
  
    }
    paint.setStyle(Paint.Style.STROKE);
    canvas.drawText(String(MainGesturesAry.length + "步动作"), 80, textSize*2, paint)
    ...
    if (TouchPointStart.length) {
        ...
        for (let i = 0; i < TouchPointStart.length; i++) {
            try {
                ...
                canvas.drawText(String("A点：X:"+X+",Y:"+Y), 30, textSize*3, paint)
        ...
        for (let ii = 0; ii < TouchPointRecord.length; ii++) {
           ...
           for (let i = 2; i < ge.length - 1; i++) {
                ...
                //let rect3 = new android.graphics.RectF(x, y, a, b);
                //canvas.drawRect(rect3, paint);
                //canvas.drawColor(1, android.graphics.PorterDuff.Mode.CLEAR);
              //  canvas.drawText(String(" X:"+X+",Y:"+Y), 40, textSize*3, paint)
            ...
            default:
                ...
                switch (event.getActionMasked()) {
                    case event.ACTION_DOWN:
                        //第一个手指按下。
                        // log("down");
                        log('间隔' + (new Date() - start) + 'ms')
                        let ms = (new Date() - start)

                        if (isNaN(ms) == true || ms.toString().length > 6) {
                            ms = 0;
                        }
                        ...
                        TouchPointRecord.splice(I, 0, [ms, 1, [Math.floor(X + vrx), Math.floor(Y + vry)]]);
                        TouchPointStart.splice(I, 0, [X, Y]);
                        TouchPointCurrent.splice(I, 0, [X, Y]);
                        break;
                    case event.ACTION_UP:
                        //最后一个手指抬起。
                        //log("up");                        start = new Date()
                        kg = false;
                        jishi = 0;
                        ...
                        TouchPointCurrent = new Array;
                        TouchPointRecord = new Array;
                        MainGesturesAry[0][0][0] = 1;
                        toastLog("动作执行中...\n在此期间，不要触碰屏幕");
                        ui.run(() => {
                            //不拦截
                            settouch(false)
                            win_.but_p.setText("动作执行中")
                        });
                        threads.start(function() {
                            if (!isAuto) {
                                return;
                            ...
                            let i_action = JSON.parse(JSON.stringify(MainGesturesAry[MainGesturesAry.length - 1]))
                            i_action[0][0] = 0;

                            sleep(250);
                            gestures.apply(null, i_action);
                            sleep(250);

                            ui.run(() => {
                                win_.but_p.setText("运行录制")
                                win_.setTouchable(true);
                                if (win_.but_l.getText() == "关闭录制") {
                                    //拦截
                                    settouch(true)
