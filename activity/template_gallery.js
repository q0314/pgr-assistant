...
log(mainScriptPath)
require(mainScriptPath + '/utlis/Runtimes.js')(global)
require(mainScriptPath + '/utlis/ResourceMonitor.js')(runtime, this)

let {
    filesx
} = require(mainScriptPath + '/modules/ext-files');
let indexFilePath = mainScriptPath + "/vue_configs/template_gallery.html";
const prepareWebView = require(mainScriptPath + '/utlis/PrepareWebView.js')
...
let gallery = require("./gallery.js");
let storageConfig = storages.create("pgr_gallery_config");
...
let galleryInfo = {
    ...
    dir: undefined,
    zip: files.path("../library/gallery/template/template.zip"),
    name: "pgr内置模板",
    cache: undefined,
};
...
let bridgeHandler = {
    ...
    cropTool:data=>{
        engines.execScriptFile(mainScriptPath+"/工具/图色助手/main.js");
    },
    ...
    switch_gallery: (data, callbackId) => {
        ...
        if (_galleryInfo) {
            ...
            postMessageToWebView({
                functionName: 'doUpdateImageInfo'
            })
