...
let gallery = {
    ...
    language: false,
    name: null,
    path: files.path("../library/gallery/"),//context.getExternalFilesDir(null).getAbsolutePath() + "/gallery_list/",
    init() {
        ...
        try {
            for (let a_tuku in tukuss) {
                ...
                if (files.exists(zipfile_name + ".zip")) {
                    ...
                    if (files.exists(zipfile_name + "/tuku/gallery_message.json")) {
                ...
                switch (item.state) {
                    case "查看图库文件信息":
                        ...
                        let gallery_info_json = JSON.parse(
                            files.read(
                                files.exists(this.path + item.file_name + "/tuku/gallery_message.json") ?
                                this.path + item.file_name + "/tuku/gallery_message.json" :
                                this.path + item.file_name + "/gallery_message.json",
                                (encoding = "utf-8"))
                        );
    ...
    unzip_copy(p1, p2) {
        ...
        try {
            ...
            if (zipfile.getFileHeader(tuku ? "tuku/gallery_info.json" : "gallery_message.json")) {
                console.verbose("解压图库相关信息文件到源目录:" + p1);
                zipfile.extractFile(tuku ? "tuku/gallery_message.json" : "gallery_message.json", p1);
                if (tuku) {
                    files.move(p1 + "tuku/gallery_message.json", p1 + "gallery_message.json");
                }
            } else {
