...
FileChooser.prototype.renderView = function () {
    ...
    view.fileList.on("item_click", (item, i, itemView) => {
        ...
        if (item.isDir) {
           ...
           if(device.sdkInt >= 30){
            if(this.currentDir == files.path("/sdcard/Android/data")){
                toastLog("安卓11以上无法访问该目录\n/sdcard/Android/data/")
            }
           }
...
FileChooser.prototype.listFiles = function (dir) {
    ...
    let list = Array.prototype.map.call(files.listDir(dir), (name) => {
        ...
        let bg_color = "#ffffff";
        if(name.indexOf(options.mimeType) !=-1 &&name.indexOf(options.mimeType)>name.length-5){
          bg_color = "#eeeac0"
      }
        return {
            fileName: name,
