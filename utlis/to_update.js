...
let {
    project
} = require('../modules/mod-project');


let toupdate = {
    showProgress() {
        if (this.progressDialog && this.progressDialog.isShowing()) {
    ...
    updata: function(uix) {
        let {
            appx
        } = require('../modules/ext-app');
        let {
            dialogsx
        } = require('../modules/ext-dialogs');
        let _updateDialog = this.progressDialog;

        return new Promise((resolve, reject) => {
            project.getNewestReleaseCared({
                min_version_name: 'v1.0.1',
                ...
                if (newest) {
                    if (!appx.version.isNewer(newest, project.getLocalVerName())) {
                        ...
                        if (_updateDialog && _updateDialog.isShowing()) {
                            ...
                            dialogsx.builds([
                                '版本详情', newest.brief_info_str, ['忽略此版本', 'warn'], 'B', ['立即更新', 'attraction'], 1,
                            ]).on('neutral', (d) => {
                    ...
                    _updateDialog && _updateDialog.setContent("网络异常，无法访问更新资源");
                    toastLog("无法访问更新资源");
                    //  resolve(true);
